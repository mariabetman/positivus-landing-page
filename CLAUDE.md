...
- **Quando o componente tem `variants/` e/ou algum valor de `data-prop-modifier` com CSS escrito, e o `.js` já existe**, o preview troca completamente de estratégia: em vez de montar o Shadow DOM na mão, ele carrega o `.js` **do próprio componente** (mesmo mecanismo do item acima) e estampa uma tag real por combinação de eixos (produto cartesiano, incluindo a combinação toda-padrão), cada uma rotulada (ex: `variant=compact`, `appearance=dark`) — o navegador faz a composição de verdade via `BaseComponent`/`import.meta.glob`, sem duplicar essa lógica no plugin. Os eixos de `data-prop-modifier` são lidos direto do `.html` (acha o elemento marcado, pega a classe-base) e do `.css` (acha as regras `.classe-base--valor` já escritas) do componente, sem precisar rodar nenhum componente de verdade pra descobrir isso — mesma ideia de `readVariantAxes`, só que pra modificador em vez de arquivo de variante (ver "Criar um novo valor de `data-prop-modifier`" em `component-props.md` pro comando que já cria a regra CSS pronta pra preencher). Sem nenhum eixo (nem `variants/`, nem modificador com CSS), ou antes do `.js` existir, o comportamento continua sendo o de sempre (Shadow DOM montado na mão a partir do `.html`/`.css`). `data-prop-class` continua fora dessa enumeração (aceita classe livre, sem valor fixo pra listar).
...
`npm run generate:composition-imports` (roda `scripts/generate-component-composition-imports.js`) automatiza esse `import`. Comando manual, independente dos outros:
...
- Mesma regra de commit dos outros scripts: `feat: adiciona import de composição em positivus-<nome>` (ou `feat: adiciona import de composição na index` pro `main.js`), sem escopo.

## Geração automática de valor de `data-prop-modifier`
